...
const config = {
  PORT: 3000,
  TOKEN_SIGN_KEY: "tokendimongodb1234",
  MONGODB_URI: `mongodb+srv://claudioamendola:mt4p3bxT066cl5N0@clustercorso.836vmvs.mongodb.net/`,
...
app.use(function (req, res, next) {
  ...
  if (
    ...
    res.status(400).json({ rc: 1, msg: "Manca il token" });
  }
});
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    // if (!user)
    //   return res.status(404).json({ rc: 1, msg: `User ${username} not found` });
    ...
    // if (!match)
    //   return res.status(401).json({ rc: 1, msg: "Invalid credentials" });
...
app.get("/listMovies", async (req, res) => {
  ...
  try {
    ...
    const filter = {};
    if (title) {
      filter.title = { $regex: title, $options: "i" }; // cerca anche titoli parziali
    }
    ...
    res.status(500).json({
      rc: 1,
      msg: "C'è stato un errore. Riprova più tardi.",
    });
  } finally {
    await client.close();
  }
